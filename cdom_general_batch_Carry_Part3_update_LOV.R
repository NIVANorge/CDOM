...
#### OBS Louise! Husk: Restart R-programmet dersom du har jobbet med flere skripts!
...
folderin<-"cdom_2021/processed/" 
folderout_results<- "cdom_2021/results/"
folderout_plots<- "cdom_2021/plots/models/"

#Add logbook (check for latest version on K)
#logbook <- read_excel("log_cary.xlsx")
logbook <- read.delim ("log_cary_210914.txt")
...
for(file in filelist){
  ...
  df<- as.data.table(df) 
  #tester å fjerne error
  mod1<- try(cdom_fit_exponential(df$wl,df$absorption, wl0 = 443,startwl=400,endwl=550 ), silent = T) # 6) fit a model marine data
  if(isTRUE(class(mod1) == "try-error")) { next }
  mod2<- try(cdom_fit_exponential(df$wl,df$absorption, wl0 = 320,startwl=275,endwl=295 ), silent = T) # 6) fit a model freshwater data
  if(isTRUE(class(mod2) == "try-error")) { next }
  mod3<- try(cdom_fit_exponential(df$wl,df$absorption, wl0 = 320,startwl=350,endwl=400 ), silent = T) # 6) fit a model freshwater data
  if(isTRUE(class(mod3) == "try-error")) { next }
...
#save summary files
write.table(dfSummary2,file=paste0(folderout_results,"/","results_all_spectras_211129.csv"),sep=",",row.names=F,quote=F)
write.table(dfSummary2,file=paste0(folderout_results,"/","results_all_spectras_211129.txt"),sep=",",row.names=F,quote=F)
